...
export async function addProduct(formData) {
  ...
  try {
    ...
    const sanitizedPrice = String(productData.currentPrice).replace(
      /[^0-9.]/g,
      "",
    );
    ...
    if (isNaN(newPrice)) {
      console.log(productData, "productData");
      return { error: "Could not extract a valid price from this URL" };
    }

    const currency = productData.currencyCode || "EUR"; // Default to EUR if currency code is not available
